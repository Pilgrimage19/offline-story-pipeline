...
### 9.0 当前 MVP 实测（2026-09-06）

使用《流浪地球》7 条人工标注 query（其中 6 条计入 Recall@5，1 条仅检查防剧透）进行对比：

| 后端 | 命中 | Recall@5 | 防剧透违规 |
|---|---:|---:|---:|
| LanceDB + bge-m3 | 5/6 | 83.3% | 0 |
| SQLite FTS5 | 2/6 | 33.3% | 0 |

这是小样本方向性结果，不是最终 benchmark；“氦闪”仍是 LanceDB 的 bad case，需要真实 LLM 摘要和更完整的标注集继续验证。

### 9.0.1 LLM 小批量成本试跑

使用 `deepseek-v4-flash` 对前 54 个 unit 试跑：77 次请求（含重试和 1 次章节压缩）、输入 146,749 token、输出 67,964 token、最终降级 12/54。代码会将服务端 usage 写入 `03_extracted/meta.json` 的 `llm_usage`；正式整本运行前应先降低降级率。

追加对照试验：将单次请求的最大总尝试次数从 2 调为 3（首次 + 2 次重试），对前 20 个 unit 试跑，实际 34 次请求、输入 47,802 token、输出 33,009 token，最终降级 4/20（20%）。降级率仅小幅下降，说明重试对偶发失败有效，但无法根治模型输出截断/非 JSON；后续仍需继续优化输出长度和上下文预算。试验产物不作为最终整本数据，需在方案确定后重新抽取。

上下文检索对照试验：在精简 state + 最近 1～2 段原文之外，追加当前 chunk 之前最多 3 个 lexical 相关历史片段（约 1800 字上限），前 20 个 unit 使用相同的 3 次总尝试配置试跑。结果为 40 次请求、输入 66,750 token、输出 40,101 token，降级 7/20（35%）。相较不追加历史的 20% 降级率明显变差，说明当前简单拼接原文的方式没有减少 prompt，反而增加了输出压力；该功能保持实验开关关闭，不作为正式上下文方案。

多阶段抽取试验：核心抽取成功后追加一次短 prompt 复核，复核只返回 patch（修正项），由代码应用到核心结果；复核失败保留核心结果而不降级。前 5 个 unit（3 次总尝试、关闭额外历史检索）结果为 22 次请求、输入 17,309 token、输出 18,297 token，最终降级 0/5；复核本身多次出现非 JSON，说明它降低了最终降级但增加了请求和 token。当前已将复核输出进一步收缩为 patch，开始完整运行前需重新验证。

完整抽取（最终试验配置）：108 个 unit 使用三路独立任务（summary/entity/relation）、DeepSeek-V4-Flash 非思考模式、最多 3 次总尝试，关闭额外历史检索和复核。结果为 328 次请求、输入 1,073,656 token、输出 27,483 token、最终降级 0/108；章节压缩 4 次，`llm_errors.json` 为空。自动结构校验通过，向量评测在 6 个可评分案例上 Recall@5=100%，无剧透违规。

当前质量观察：摘要和实体字段已可用，但剧情线仍存在相近标题分裂（例如“环球旅行课程”与“小学环球旅行课程”、“太阳氦闪爆发”与“太阳氦闪危机”），以及少量组合/泛化实体，需要后续做 canonical entity/plotline 归并和人工抽检；这些不影响当前索引构建，但尚未达到设计冻结标准。
