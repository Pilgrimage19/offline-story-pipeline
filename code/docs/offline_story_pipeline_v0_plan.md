...
多阶段抽取试验：核心抽取成功后追加一次短 prompt 复核，复核失败保留核心结果而不降级。前 5 个 unit（3 次总尝试、关闭额外历史检索）结果为 22 次请求、输入 17,309 token、输出 18,297 token，最终降级 0/5；复核本身多次出现非 JSON，说明它降低了最终降级但增加了请求和 token，后续需优化复核 prompt/格式约束后再扩大规模。
